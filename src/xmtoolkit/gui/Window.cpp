...
Window::Window(const char* title) :
		mParent(NULL), mChild(NULL), mIsModal(false), mCloseType(HIDE_ON_CLOSE), mLocks(
				0), mWindowListener(0), mIsDeleteable(true), mIsVisibleInTaskBar(
	...
	if (!mainWindow) {
		...
		mainWindow = CreateWindowExW(0, CLASS_NAME, NULL,
		WS_OVERLAPPEDWINDOW, 0, 0, 0, 0, 0, 0, instance, 0);
	}
	wchar_t *realTitle = toUTF16(title);
	this->setHwnd(CreateWindowExW(
	WS_EX_APPWINDOW, CLASS_NAME, realTitle,
	WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN,
	CW_USEDEFAULT, CW_USEDEFAULT, 200, 200, 0, NULL, instance, NULL));
	delete realTitle;
	topWindowsCount++;
	ventanas.add(this);
	SendMessage(*this, WM_SETFONT, (WPARAM) GetStockObject(DEFAULT_GUI_FONT),
			TRUE);
...
void Window::setFullScreen(bool fullscreen) {
	...
	if (mIsFullScreen) {
		...
		WS_EX_STATICEDGE);
		//Movemos la ventana
		SetWindowPos(*this, NULL, info.rcMonitor.left, info.rcMonitor.top,
	...
	if (ventana) {
		...
		case WM_GETMINMAXINFO: {
			...
			info->ptMaxTrackSize.x =
					maxW <= 0 ? GetSystemMetrics(SM_CXMAXTRACK) : maxW;
			info->ptMaxTrackSize.y =
					maxH <= 0 ? GetSystemMetrics(SM_CYMAXTRACK) : maxH;
			info->ptMinTrackSize.x =
					minW <= 0 ? GetSystemMetrics(SM_CXMINTRACK) : minW;
			info->ptMinTrackSize.y =
					minH <= 0 ? GetSystemMetrics(SM_CYMINTRACK) : minH;
			return 0;
			break;
		}
		case WM_ACTIVATE: {
